...
% {shape} {material} spheroid in {medium}
% semi-axes a=b=20nm, c=40nm
% semi-axes a=b={a}nm, c={c}nm
wavelength = 350:{step}:850; wavelength = wavelength(:); 
Nl = length(wavelength);
epsilon=epsilon_{material}(wavelength);
medium={n};
...
epsilon = struct(...
    ...
    'material_name', '{material}', 'material_reference', '{source}',...
    'material_keywords', 'dispersive');

geometry = struct('description',  '{shape} spheroid', ...
...
comments = struct('name', '{material} {shape} spheroid in water',...
    'description', 'Computation using SMARTIES, a numerically robust EBCM implementation for spheroids',...
    'keywords', '{material}, spheroid, ebcm', 'script', [mfilename '.m']);

tmatrix_hdf5('smarties_{material}_{medium}_{a}_{c}.tmat.h5', tmatrix, wavelength, epsilon,...
