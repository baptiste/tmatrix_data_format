...
Arranging the expansion coefficients in this order naturally leads to the common `2x2` block-matrix convention. Explicitly, for a T-matrix with $l_\text{max}=3$, we can visualise the 900 elements organised as follows:
...
$$u = 2(p-1) + (3-s)$$
where $(3-s)$ swaps electric and magnetic ordering (1 is electric in tmat.h5, 2 magnetic, but in the original block-matrix layout the first index is magnetic). Noting this little change, the index $u$ simply zips odd/even terms from the $p$ values of electric/magnetic blocks. Below is a reference table for the conversions taking place for the example of $l_\text{max}=3$. The index $q$ runs from 1 to 30, and the corresponding index $u$ in tmat.h5 convention has the elements reordered.
